...
aclError ModelProcess::Init(std::string modelPath)
{
    ...
    //get current context
    cout << "aclrtGetCurrentContext." << endl;
    ...
    aclmdlIODims dims;
    ret = aclmdlGetInputDims(modelDesc, 0 , &dims);
    if (ret != ACL_ERROR_NONE) {
        cout << "get input dims failed,  errorCode is  " << static_cast<int32_t>(ret)<< endl;
        return ret;
    }
